<!-- src/views/PlanificacionDetalle.vue -->
<template>
  <ion-page>
    <TopBar />
          ...
          <ion-card-header>
            <div class="header-row">
              ...
              <div v-if="isCoach" class="action-wrap">
                <ion-button size="small" fill="outline" color="dark" class="icon-btn" @click="toggleEditarPlan">
                  <ion-icon :icon="editando ? closeOutline : createOutline" />
                  <span class="btn-text">{{ editando ? 'Cerrar' : 'Editar' }}</span>
                </ion-button>
                <ion-button size="small" fill="outline" color="danger" class="icon-btn" @click="openConfirmPlanDelete">
...
const cargar = async () => {
  try {
    plan.value = await apiPlanif.detalle(route.params.id)
  } catch {
...
.action-wrap, .header-actions, .row-actions { display: flex; gap: 8px; align-items: center; }
